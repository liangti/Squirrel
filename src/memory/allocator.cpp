...
public:
  ...
  void deallocate(word_t *p) { manager.free_block(p); }
  ~AllocatorImpl() {}
};

AllocatorBase::AllocatorBase() { impl = new AllocatorImpl(); }

word_t *AllocatorBase::allocate(size_t size) { return impl->allocate(size); }
